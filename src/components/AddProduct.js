...
const AddProduct = () => {
    ...
    const [product, setProduct] = useState({ title: "", description: "", tag:"all", imgURL: "null" })

    const handleClick = (e) => {
        e.preventDefault();
        addProduct(product.title, product.description, product.tag, product.imgURL);
    }
        ...
        <div className="container my-3 add-product-container">
            ...
            <form className="my-3 row" id="add-product-form">
                <div className="row add-product-item-container">
                    ...
                    <button type="submit" className="btn btn-success " id='add-product-btn' onClick={handleClick}>Submit</button>
                </div>
            </form>
        </div>
    )
